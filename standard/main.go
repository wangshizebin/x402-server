...
func loadConfig() *Config {
	...
	if config.NodeEnv == "production" {
		...
		if corsOrigins == "" {
			log.Println("⚠️  CORS_ORIGINS not set in production. Using empty array (no CORS allowed).")
...
func setupCORS(r *gin.Engine, config *Config) {
	r.Use(func(c *gin.Context) {
		...
		// Set CORS headers for all responses (including 402, 401, 403, etc.)
		// Use Writer.Header() to ensure headers are set even if middleware aborts
		if allowed {
			if origin != "" {
		...
		// After response is written, ensure CORS headers are still present
		// This is critical for all responses including 402, 401, 403, etc.
		if allowed {
			if origin != "" {
				c.Writer.Header().Set("Access-Control-Allow-Origin", origin)
			}
